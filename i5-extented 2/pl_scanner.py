...
class Scanner(object):
    ...
    #symbols = set("@[\]^_`!\"#$%&',)(*+-./:;<=>?")
    operators = {'=','+','-','*','/','(',')',';','<','<=','>','>=','<>','==', '&', '|', '^', '~', '<<', '>>', '{', '}'}
    keywords = {'rd', 'wr', 'if', 'then', 'else', 'while', 'do', 'begin', 'end', 'def'}
    allowedString =  set("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") 
    token = ""
    lexeme = ""
    strFound = False
